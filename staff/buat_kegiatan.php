...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama_kegiatan) || empty($waktu_mulai) || empty($waktu_selesai)) {
        ...
        $kegiatanData = [
            ...
            'deskripsi' => $deskripsi,
            'status' => 'berjalan', // status awal ketika dibuat, asumsikan langsung berjalan atau dijadwalkan
            'created_at' => date('Y-m-d H:i:s')
        ];

        $res = firebase_push('/activities', $kegiatanData);
        if ($res) {
            $_SESSION['success_msg'] = "Kegiatan berhasil ditambahkan.";
            header("Location: dashboard.php");
            exit;
        } else {
            $_SESSION['error_msg'] = "Gagal menyimpan ke Firebase.";
        }
    }
}
...
<div class="mb-8 animate-fade-in">
    <h2 class="text-3xl font-extrabold text-slate-800 tracking-tight">Buat Jadwal Kegiatan</h2>
    <p class="text-slate-500 mt-1">Isi formulir di bawah ini untuk merencanakan kegiatan Anda.</p>
</div>
...
<div class="bg-white/80 backdrop-blur-xl shadow-xl shadow-slate-200/50 rounded-3xl p-8 max-w-2xl border border-slate-100 animate-slide-up">
    ...
    <form action="buat_kegiatan.php" method="POST" class="space-y-5">
        <div>
            <label class="block text-slate-700 text-sm font-bold mb-2">Nama Kegiatan</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                <label class="block text-slate-700 text-sm font-bold mb-2">Waktu Mulai</label>
            ...
            <div>
                <label class="block text-slate-700 text-sm font-bold mb-2">Waktu Selesai</label>
        ...
        <div>
            ...
            <textarea name="deskripsi" rows="4" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all duration-300 placeholder-slate-400" placeholder="Tulis rincian singkat kegiatan..."></textarea>
        </div>
